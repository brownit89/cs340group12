...
router.post('/', function(req, res){
	console.log(req.body)
	var mysql = req.app.get('mysql');
	var minionSql = "insert into minion (attack_power, health_power) values (?, ?);";
	var minionInsert = [req.body.attack_power, req.body.health_power];
	minionSql = mysql.pool.query(minionSql,minionInsert,function(error,results,fields){
		if(error){
			console.log(JSON.stringify(error))
			res.write(JSON.stringify(error));
			res.end();
...
router.delete('/:id', function(req, res){
	...
	var minionSql = "delete from minion where minion_id = ?";
	var cardInsert = [req.params.card_id];
	var minionInsert = [req.params.minion_id];
	minionSql = mysql.pool.query(minionSql, minionInsert, function(error, results, fields){
		if(error){
		console.log(error)
		res.write(JSON.stringify(error));
